...
InitStatus FrsCal2Hit::Init()
{
    ...
    fCalArray = (TClonesArray*)mgr->GetObject("FrsCalData");
    c4LOG_IF(fatal, !fCalArray, "FrsCalData branch not found!");

    mgr->Register("FrsHitData", "FRS Hit Data", fHitArray, !fOnline);

    SetParameters(); // ah yes we need an actual argument..

    fCalArray->Clear();
    fHitArray->Clear();

    //std::cout << sci->mhtdc_factor_21l_21r << std::endl;
...
{   
    ...
    // do stuff man
    int mult = fCalArray->GetEntriesFast();

    if (mult!=0)
    {   
        fNEvents++;
        fCalHit = (FrsCalData*)fCalArray->At(0);

        WR_TS = fCalHit->Get_WR();

        // -----------------------------------
        // V792 // eventually gets to id_x2 etc
        // -----------------------------------
        de_array = fCalHit->Get_De_array();
        ...
        if (b_tpc_xy[4] && b_tpc_xy[5]) // these array elements may change for tpc stuff. not in place yet
        {
            id_x4 = tpc_x_s4;
            id_y4 = tpc_y_s4;
        ...
        // V1290
        tdc_array = fCalHit->Get_TDC_array();
