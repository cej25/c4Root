...
FrsCal2Ana::FrsCal2Ana()
    ...
    ,   tpatArray(nullptr)   
    ,   scalerArray(nullptr)
    ,   anaArray(new std::vector<FrsAnaItem>)
    ,   multihitAnaArray(new std::vector<FrsAnaMultiItem>)
{
...
InitStatus FrsCal2Ana::Init()
{
    ...
    mgr->RegisterAny("FrsAnaData", anaArray, !fOnline);
    mgr->RegisterAny("FrsAnaMultiData", multihitAnaArray, !fOnline);
...
{   
    ...
    auto & anaEntry = anaArray->emplace_back();
    ...
    for (int i = 0; i < hits_in_beta_s2s4; i++)
    {
        for (int j = 0; j<hits_in_s2x; j ++)
        {
            ...
            {
                auto & multihitAnaEntry = multihitAnaArray->emplace_back();
                multihitAnaEntry.SetAll(temp_s2x_mhtdc[j],
                                    temp_a2,
                                    temp_s4x,
                                    temp_a4,
                                    id_mhtdc_tof_s2s4[i],
...
void FrsCal2Ana::ProcessSci_TAC()
{
...
void FrsCal2Ana::ProcessSci_MHTDC()
{
    auto const & calSciItem = calSciArray->at(0);
...
void FrsCal2Ana::ProcessIDs_MHTDC()
{
    auto const & calTpcItem = calTpcArray->at(0);
    b_tpc_xy = calTpcItem.Get_b_tpc_xy();
...
void FrsCal2Ana::ProcessDriftCorrections()
{
