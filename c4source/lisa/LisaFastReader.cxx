...
extern "C"
{
    ...
    #include "ext_h101_lisafast.h"
}

/*
Constructor. Needs the input data structure obtained from ucesb with the following args:
 --ntuple=UNPACK:lisafast,STRUCT_HH,ext_h101_lisafast.h (add NOEVENTHEAD also).
 
 lisafast/lisafast --ntuple=UNPACK,NOTRIGEVENTNO,STRUCT_HH,id=h101_lisafast,ext_h101_lisafast.h
...
*/
LisaFastReader::LisaFastReader(EXT_STR_h101_lisafast_onion* data, size_t offset)
    : c4Reader("LisaFastReader")
    , fNEvent(0)
    , fData(data)
    , fOffset(offset)
    , fOnline(kFALSE)
    , fArray(new TClonesArray("LisaFastData"))
{
}
...
LisaFastReader::~LisaFastReader() { 
    ...
    c4LOG(info, "Destroyed LisaFastReader properly.");
...
*/
Bool_t LisaFastReader::Init(ext_data_struct_info* a_struct_info)
{
    Int_t ok;

    EXT_STR_h101_lisafast_ITEMS_INFO(ok, *a_struct_info, fOffset, EXT_STR_h101_lisafast, 0);
    ...
    FairRootManager::Instance()->Register("LisaFastData", "LisaFastDataFolder", fArray, !fOnline);
    fArray->Clear();
...
*/
void LisaFastReader::DoFineTimeCalibration()
{
...
*/
double LisaFastReader::GetFineTime(int tdc_fine_time_channel, int board_id, int channel_id)
{
...
*/
void LisaFastReader::WriteFineTimeHistosToFile()
{
    ...
    LOG(info) << Form("LisaFast fine time calibrations (i.e. raw fine time histograms) written to  %s",fine_time_histo_outfile.Data());

    outputfile->Close();

    c4LOG(info,"You have successfully done fine time calibration. These are written to file. Please restart the program and add ReadFineTimehistosFromFile instead for LisaFastReader.\n (yeah this is not a fatal error per se just restart it with your fresh calibrations :) )");
...
*/
void LisaFastReader::ReadFineTimeHistosFromFile() 
{
    ...
    inputfile->Close();
    LOG(info) << Form("LisaFast fine time calibration read from file: %s", fine_time_histo_infile.Data());
}
...
Bool_t LisaFastReader::Read() //do fine time here:
{
    ...
    { //per board:
        ...
        lisafast_last_lead_hit_struct last_tdc_hit; // initialized and reset - this keeps the information of the last lead that was seen
        ...
        //c4LOG(info,Form("event size: %i",fData->lisafast_tamex[it_board_number].event_size));
        //c4LOG(info,Form("time_epoch =  %i, time_coarse = %i, time_fine = %i, time_edge = %i, time_channel = %i",fData->lisafast_tamex[it_board_number].time_epoch,fData->lisafast_tamex[it_board_number].time_coarse,fData->lisafast_tamex[it_board_number].time_fine,fData->lisafast_tamex[it_board_number].time_edge,fData->lisafast_tamex[it_board_number].time_channel));
        //c4LOG(info,"Here comes data:");

        for (int it_hits = 0; it_hits < fData->lisafast_tamex[it_board_number].event_size/4 - 3 ; it_hits++)
        {
            ...
            */
            ...
            if (fData->lisafast_tamex[it_board_number].time_fine <= it_hits) {fNevents_skipped++; continue;}
            if (fData->lisafast_tamex[it_board_number].time_coarse <= it_hits) {fNevents_skipped++; continue;}
            //the length of the arrays should be equal. 
            if (!(fData->lisafast_tamex[it_board_number].time_coarse == fData->lisafast_tamex[it_board_number].time_fine && fData->lisafast_tamex[it_board_number].time_coarse == fData->lisafast_tamex[it_board_number].time_epoch)) {fNevents_skipped++; continue;}
            ...
            if (fData->lisafast_tamex[it_board_number].time_epochv[it_hits] != 0)
            {
                    previous_epoch_word = fData->lisafast_tamex[it_board_number].time_epochv[it_hits] & 0xFFFFFFF;
                    //if (it_board_number == 1) c4LOG(info,Form("Found epoch for ch = %i, e = %i",next_channel,fData->lisafast_tamex[it_board_number].time_epochv[it_hits] & 0xFFFFFFF));
                    fNepochwordsread++;
            ...
            uint32_t channelid = fData->lisafast_tamex[it_board_number].time_channelv[it_hits] & 0x7F; // 0-32
            //if (it_board_number == 1) c4LOG(info,Form("ch = %i, coarse = %i, edge = %i", channelid, fData->lisafast_tamex[it_board_number].time_coarsev[it_hits], fData->lisafast_tamex[it_board_number].time_edgev[it_hits]));

            if (fData->lisafast_tamex[it_board_number].time_finev[it_hits] == 0x3FF) {fNevents_TAMEX_fail[it_board_number][channelid]++; continue;} // this happens if TAMEX loses the fine time - skip it
            ...
            bool is_leading = fData->lisafast_tamex[it_board_number].time_edgev[it_hits] & 0x1;
            //c4LOG(info,"is leading : " << is_leading);

            //Fill fine times and skip.
            if (!fine_time_calibration_set && is_leading)
            {
                fine_time_hits[it_board_number][channelid]->Fill(fData->lisafast_tamex[it_board_number].time_finev[it_hits]);
                continue;
            }
            ...
            { 
                ...
                new ((*fArray)[fArray->GetEntriesFast()]) LisaFastData(
                    trig,
                    it_board_number,
                    channelid,
...
*/
void LisaFastReader::PrintStatistics()
{
    std::ostringstream oss;
    // Print column labels
    oss << "\n";
    oss << ANSI_COLOR_GREEN << "LISA-FAST TAMEX4 STATISTICS - boards from 0-N. TDC channels are numbered from 1-32 fast(odd)-slow(even)" << ANSI_COLOR_RESET;
    oss << "\n";
