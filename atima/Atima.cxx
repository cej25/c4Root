...
void Atima::Calculate(catima::Projectile p, catima::Material t, Float_t e)
{
...
void Atima::BuildMaterial()
{
    ...
    // SCI21 - EJ230
    AddLayer(Material({{1,1,10}, {12,6,9}}, 1.032, 0.1416936));
    // TPC22 pocket Ti
    ...
    if (!printed) 
    {
        ...
        // /*if(frs_config->degrader_in == true)*/std::vector<std::string> names = {
        ...
        std::vector<std::string> names = {
            "SCI21", "TPC22 pocket", "TPC22 gas", "Vacuum window",
            "Air #1", "TPC23 windows", "TPC23 gas", "Air #2",
            "MUSIC21 windows", "MUSIC21 gas", "Air #3", "LISA window"
        };
...
std::pair<Float_t, Float_t> Atima::CalculateEnergyLoss(const catima::Projectile& proj, Float_t E_initial)
{
    Float_t E_in = E_initial;
    Float_t total_loss = 0.0;
    ...
    for(size_t i = 0; i < shiyan_s1s2.size(); ++i)
    {
        ...
        Float_t E_out = result.Eout;
        Float_t dE = E_in - E_out;
