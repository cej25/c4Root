...
void TreeReader()
{
    ...
    ImplantTree.Add("~/lustre/gamma/jeroen/S100/cluster/trees/168Dy/168Dy_0021_0004.root");
    ...
    TH1F* h1_implant_rate = new TH1F("h1_implant_rate", "Implant rate", 3000, 0, 300);
    TH1F* h1_decay_rate = new TH1F("h1_decay_rate", "Decay rate", 3000, 0, 300);
    TH1F* h1_decay_rate_offspill = new TH1F("h1_decay_rate_offspill", "Decay rate Offspill", 3000, 0, 300);
    TH1F* h1_decay_rate_onspill = new TH1F("h1_decay_rate_onspill", "Decay rate Onspill", 3000, 0, 300);
    ...
    TTreeReaderValue<Bool_t> tr_spill_imp(imp_tr, "EventHeader.fSpillFlag");
    ...
    int64_t first_decay_time = 1713756192679047800;
    ...
    while (imp_tr.Next())
    {
        ...
        for (auto const & i : tr_implants)
        {
            ...
            if (*tr_spill_imp == 0)
            {
                //std::cout << "implant off spill... " << std::endl;
                imp_off++;
                continue;
            }

            int64_t t_since_first = implant.Time - first_decay_time;
            h1_implant_rate->Fill(t_since_first / 1e9);
    ...
    std::cout << "Implants found offspill:"  << imp_off << std::endl;
    ...
    while (dec_tr.Next())
    {
        for (auto const & d : tr_decays)
        {
            ...
            if (n_d == 0) std::cout << decay.Time << std::endl;
            n_d++;
            ...
            if ((decay.TimeY - decay.TimeX) < FBT && (decay.TimeY - decay.TimeX) > -FBT)
            {
                if ((decay.EnergyY - decay.EnergyX) < FBE && (decay.EnergyY - decay.EnergyX) > -FBE)
                {
                    if (decay.EnergyY > minE && decay.EnergyX > minE)
                    {
                        ...
                        if (decay.ClusterSizeX > 1 || decay.ClusterSizeY > 1)
                        {
                            std::cout << "::::::: woah big nelly :::::::" << std::endl;

                            std::cout << "ClusterSizeX: " << decay.ClusterSizeX << std::endl;
                            std::cout << "ClusterSizeY: " << decay.ClusterSizeY << std::endl;
                            std::cout << "Energy: " << decay.Energy << std::endl;
                            std::cout << "EnergyX: " << decay.EnergyX << std::endl;
                            std::cout << "EnergyY: " << decay.EnergyY << std::endl;
                            std::cout << "StripX: " << decay.StripX << std::endl;
                            std::cout << "StripXMin: " << decay.StripXMin << std::endl;
                            std::cout << "StripXMax: " << decay.StripXMax << std::endl;
                            std::cout << "StripY: " << decay.StripY << std::endl;
                            std::cout << "StripYMin: " << decay.StripYMin << std::endl;
                            std::cout << "StripYMax: " << decay.StripYMax << std::endl;
                            continue;
                        }
                        ...
                        //if (t_since_start / 1e9 > 40 && t_since_start / 1e9 < 70 && *tr_spill == 0) std::cout << "this time is on spill..." << std::endl;

                        h1_decay_rate->Fill(t_since_start / 1e9);

                        // off-spill only
                        if (*tr_spill) continue;

                        h1_decay_rate_offspill->Fill(t_since_start / 1e9);
