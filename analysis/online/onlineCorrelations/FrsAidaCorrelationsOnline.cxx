        ...
        if (hit.Time - hitItemFrs.Get_wr_t() > Correl["FRS-AIDA WR Gate"][0] && hit.Time - hitItemFrs.Get_wr_t() < Correl["FRS-AIDA WR Gate"][1])
        {
            ...
            if (hit.Stopped)
            {
                if (!FrsGates.empty())
                {
                    for (int gate = 0; gate < FrsGates.size(); gate++)
                    {
                        if (FrsGates[gate]->PassedGate(hitItemFrs.Get_ID_z(), hitItemFrs.Get_ID_z2(), hitItemFrs.Get_ID_x2(), hitItemFrs.Get_ID_x4(), hitItemFrs.Get_ID_AoQ(), hitItemFrs.Get_ID_dEdeg(), hitItemFrs.Get_sci_e_42()))
                        {
