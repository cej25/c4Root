...
InitStatus bPlastOnlineSpectra::Init()
{
    ...
    gr_bplast_deadtime = new TGraph(3600);
    gr_bplast_deadtime->SetName("gr_bplast_deadtime");
    gr_bplast_deadtime->SetTitle("bPlast Deadtime");
    gr_bplast_deadtime->GetXaxis()->SetTitle("Time [s]");
    gr_bplast_deadtime->GetYaxis()->SetTitle("Deadtime [%]");
    gr_bplast_deadtime->SetMinimum(0);
    dir_bplast_hitpattern->Append(gr_bplast_deadtime);
...
{   
    ...
    if (fHitbPlastTwinpeaks && fHitbPlastTwinpeaks->GetEntriesFast() > 0)
    {
        ...
        {   
            ...
            // Deadtime calculation from pulser events
            ...
            if (fEventHeader->GetTrigger() == 3)
            {
                ...
                pulser_counter++;
                int seconds = ( wr_t - wr_prev )/ 1e9;
                // 60s timer
                timer = timer + seconds; // timer that tracks time for 1 min intervals = 60 s = 600 pulser events
                std::cout << "Timer: " << timer << std::endl;
                // 3600s timer
                graph_timer = graph_timer + seconds;
                std::cout << "Graph Timer: " << graph_timer << std::endl;
                
                // fill a TGraph every minute, after 1 hour, start overwriting the first minute
                if (timer >= 60)
                {
                    gr_bplast_deadtime->SetPoint(graph_timer, graph_timer, pulser_counter/600.0 * 100);
                    timer = 0;
                }
            }

            wr_prev = wr_t; // previous event set to next
