...
InitStatus bPlastOnlineSpectra::Init()
{
    ...
    h1_bplast_fast_hitpatterns = new TH1F("fast_HitPatt","fast_HitPatt",NDetectors,0,NDetectors);
    ...
    h1_bplast_fast_hitpatterns = new TH1F("slow_HitPatt","slow_HitPatt",NDetectors,0,NDetectors);
    h1_bplast_fast_hitpatterns->GetXaxis()->SetTitle("Detector ID");
    h1_bplast_fast_hitpatterns->GetYaxis()->SetTitle("Counts");
    h1_bplast_fast_hitpatterns->Draw();
    ...
    // Time spectra:
    c_bplast_time_spectra  = new TCanvas("bPlast_c_bplast_time_spectra","bPlast time spectra",650,350);
    c_bplast_time_spectra->Divide(5,(NDetectors%5==0) ? (NDetectors/5) : (NDetectors/5 + 1));

    for (int ihist = 0; ihist < NDetectors; ihist++){
        c_bplast_time_spectra->cd(ihist+1);
        h1_bplast_abs_time[ihist] = new TH1F(Form("bPlast_Time_spectrum_%d",ihist),Form("Time_spectrum_%d",ihist),10000,0,2.7e12);
...
{   
    if (fHitbPlastTwinpeaks && fHitbPlastTwinpeaks->GetEntriesFast() > 0)
    {
        ...
        {   
            ...
            std::cout << hit->Get_detector_id() << std::endl;
