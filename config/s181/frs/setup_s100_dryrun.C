...
//   } 
 ...
 frs->rho0[0]   = 1.; //TA-S2
  ...
  frs->bfield[0] = 10.8261;    // 197Au 
  frs->bfield[1] = 10.8261;    // 197Au
  frs->bfield[2] = 7.8494;//;11.5358;   //  197Au 
  frs->bfield[3] = 7.8494;//11.5358;   //  197Au
  ...
  // MHTDCAnalysis S2-S4
  id->mhtdc_length_sc2141 = 125.207*0.299792458;//37.204244; // 36.68196; // SCI 21-41 s450
  id->mhtdc_length_sc2241 = 122.677*0.299792458;//36.850489; //35.227152; // SCI 22-41 s450 208Pb
  id->mhtdc_vel_a_music41[0]= 526.137;//6459.55; //s496 Xe 210512
  id->mhtdc_vel_a_music41[1]= 1241.28;//-11135.1;
  id->mhtdc_vel_a_music41[2]= 0.0;//5581.45;
  id->mhtdc_vel_a_music41[3]=0.0;
  id->mhtdc_vel_a_music42[0]= 372.998;//7522.75; //s496 Xe 210512
  id->mhtdc_vel_a_music42[1]= 1329.1;//-13599.0;
  id->mhtdc_vel_a_music42[2]= 0.0;//7039.16;
...
//  MUSIC41 velocity 22.03.2024 197Au //eng run
  id->vel_a[0] =   13427.0; //  MUSIC41 velocity corr. //
  id->vel_a[1] =   -22587.0;   // 
  id->vel_a[2] =   11104.0; //
  id->vel_a[3] =   0.0;
...
// MUSIC42 velocity 22.03.2024 197Au //eng run
  id->vel_a2[0] =  14965.0; //
  id->vel_a2[1] = -26209.0; //
  id->vel_a2[2] =  13201.0;//
  id->vel_a2[3] =  0.0;
  ...
  tpc->x_offset[0][0] = -0.408463 +0.2 -3.0+3.4;//update on 2021/June/19 slitx, degr.center as ref
  tpc->x_factor[0][0] = 0.007978;
  tpc->x_offset[0][1] = 0.959454  +0.2 -3.0+3.4;
  tpc->x_factor[0][1] = 0.008105;
  tpc->y_offset[0][0] = -56.3389688 +0.85;//-55.037378 -0.6 -1.5;
  tpc->y_factor[0][0] = 0.0038418; //0.003956; //vacuum tpc is drift to bottom. positive y-factor
  tpc->y_offset[0][1] =-56.9720263 +0.85;// -55.193154 -0.6 -1.5;
  tpc->y_factor[0][1] = 0.0038732;//0.003953;
  tpc->y_offset[0][2] = -57.2758022 +0.85;//-56.659256 -0.6 -1.5;
  tpc->y_factor[0][2] = 0.0038965;//0.004082;
  tpc->y_offset[0][3] = -57.7001232 +0.85;//-55.009200 -0.6 -1.5;
  ...
  // if you want to change timeref, you need to calibrate y-position  again ! )
  tpc->x_offset[1][0] = 2.483279 +0.7 -0.5 -0.6;//update on 2021/June/19 slitx, degr.center as ref
  tpc->x_factor[1][0] = 0.007781;
  tpc->x_offset[1][1] = 0.561674 +0.7 -0.5-0.6;
  tpc->x_factor[1][1] = 0.007574;
  tpc->y_offset[1][0] = -58.1084677+0.6;//-57.558218 +1.4 -3.0;
  tpc->y_factor[1][0] = 0.0039634;//0.004107;   //vacuum tpc is drift to bottom. positive y-factor
  tpc->y_offset[1][1] = -58.7300878+0.6;//-56.781388 +1.4 -3.0;
  tpc->y_factor[1][1] = 0.0039666;//0.004016;
  tpc->y_offset[1][2] = -59.094806+0.6;//-57.216335 +1.4 -3.0;
  tpc->y_factor[1][2] = 0.0039668;//0.004024;
  tpc->y_offset[1][3] = -58.5754908+0.6;//-56.691696 +1.4 -3.0;
  ...
  // because calibration parameters (y) are valid only with timeref used during calibration.
  // if you want to change timeref, you need to calibrate y-position  again ! )
  tpc->x_offset[2][0] = 4.389925 +1.5-1.5+1.6-0.57-0.6+1.0-3.0-6.2+5.8; //11.05.22 -6.2 is to djust for disc-center //update on 2021/Mar/31 tpc calib for timeref id=2 (sc22).
  tpc->x_factor[2][0] = 0.008002;
  tpc->x_offset[2][1] = -0.136026 +1.5-1.5+1.6-0.57-0.6+1.0-3.0-6.2+5.8; //11.05.22 -6.2 is to djust for disc-center
  ...
  tpc->y_offset[3][0] = 63.4310738-1.3;//57.682383-1.5+9.0+(1.706-6.991)+1.2;
  tpc->y_factor[3][0] = -0.0040971;//-0.004033; //air tpc is drift to top. negative y-factor
  tpc->y_offset[3][1] = 63.8444714-1.3;//58.217353-1.5+9.0+(1.706-6.991)+1.2;
  tpc->y_factor[3][1] = -0.0040887;//-0.004044;
  tpc->y_offset[3][2] = 62.8678718-1.3;//57.839351-1.5+9.0+(1.706-6.991)+1.2;
  tpc->y_factor[3][2] = -0.0040725;//-0.004039;
  tpc->y_offset[3][3] = 62.9917085-1.3;//57.901361-1.5+9.0+(1.706-6.991)+1.2;
  ...
  // because calibration parameters (for y) are valid only with timeref used during calibration.
  // if you want to change timeref, you need to calibrate y-position  again ! )
  tpc->x_offset[4][0] = -0.657524+2.0 +1.8-3.8-0.25+1.5-1.1;// 19/June/2021, +1.5 Dec.02 2023
  tpc->x_factor[4][0] = 0.007779*12./11.;
  tpc->x_offset[4][1] = -1.806150+2.0 +1.8-3.8-0.25+1.5-1.1; // 19/June/2021//trust more final grid in front of IC, and correct for TPC41/42
  tpc->x_factor[4][1] = 0.007802*12./11.;
  tpc->y_offset[4][0] = 54.670698 -1.3 -0.5+0.8+0.8+1.4;// 19/June/2021
  tpc->y_factor[4][0] = -0.004075;  //air tpc is drift to top. negative y-factor
  tpc->y_offset[4][1] = 54.704890 -1.3 -0.5 +0.8+0.8+1.4;// 19/June/2021
  tpc->y_factor[4][1] = -0.004077;
  tpc->y_offset[4][2] = 55.482351 -1.3 -0.5+0.8+0.8+1.4;// 19/June/2021
  tpc->y_factor[4][2] = -0.004049;
  tpc->y_offset[4][3] = 55.628042 -1.3 -0.5+0.8+0.8+1.4;// 19/June/2021
  ...
  tpc->y_offset[5][0] = 55.137927 +1.3 +0.5-0.8-1.1+1.1;// 19/June/2021
  tpc->y_factor[5][0] = -0.004056; //air tpc is drift to top. negative y-factor
  tpc->y_offset[5][1] = 55.897006 +1.3 +0.5-0.8-1.1+1.1;// 19/June/2021
  tpc->y_factor[5][1] = -0.004060;
  tpc->y_offset[5][2] = 54.034448 +1.3 +0.5-0.8-1.1+1.1;// 19/June/2021
  tpc->y_factor[5][2] = -0.004039;
  tpc->y_offset[5][3] = 53.536067 +1.3 +0.5-0.8-1.1+1.1;// 19/June/2021
...
//TOF_SC41_SC21_TAC 21.03.24 Au engrun,  
  id->id_tofoff2 =  206389; //197454.8;  // offset (ps)
  id->id_path2   =  121371; //122486.0;  // path/c (ps)
  
  //TOF_SC42_SC21_TAC 21.03.24 Au engrun //09.12.23 U engrun
  id->id_tofoff3  = 157092; //201945; // 196914.0;   // offset (ps)
  id->id_path3    = 84169; // 123401; // 125574.9;   // path/c [ps
  ...
  id->id_tofoff5 =  254767; //178527; // 187952.5;  // offset (ps) 
  id->id_path5   =  160518; //121304; //117471.1;  // path/c (ps)
  ...
  sci->mhtdc_offset_41_21  =  19.2661-9.95+171.4 ; //ns //s450 208Pb
  sci->mhtdc_offset_42_21  =  96.6517-7.53+171.5; //ns // s452 Pb 210311
  sci->mhtdc_offset_43_21  =  0.0; //ns
  sci->mhtdc_offset_31_21  =  85.0; //ns
  // not known member to our TFRSParameter
