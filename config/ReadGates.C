...
{   
    ...
    for (const auto& filename : filenames)
    {
        ...
        if (!cutg) cutg = dynamic_cast<TCutG*>(gROOT->FindObject("CUTG"));
        if (cutg)
        {
            if (strcmp(type,"ZvsAoQ") == 0)
            {
                cutg->SetVarX("AoQ");
                cutg->SetVarY("Z");
                cutg->SetName(Form("ZvsAoQ%d",i));
            }
            else if (strcmp(type, "Z1vsZ2") == 0)
            {
                cutg->SetVarX("Z1");
                cutg->SetVarY("Z2");
                cutg->SetName(Form("Z1vsZ2%d",i));
            }
            else if (strcmp(type, "x2vsAoQ") == 0)
            {
                cutg->SetVarX("AoQ");
                cutg->SetVarY("x2");
                cutg->SetName(Form("x2vsAoQ%d",i));
            }
            else if (strcmp(type,"x4vsAoQ") == 0)
            {
                cutg->SetVarX("AoQ");
                cutg->SetVarY("x4");
                cutg->SetName(Form("x4vsAoQ%d",i));
            }
            else if (strcmp(type, "dEdegvsZ") == 0)
            {
                cutg->SetVarX("Z");
                cutg->SetVarY("dEdeg");
                cutg->SetName(Form("dEdegvsZ%d",i));
            }
